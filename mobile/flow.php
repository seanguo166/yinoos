...
$consignee['region'] = $db->getOne($sql); 
   ...
   $smarty->assign('shop_province_list', get_regions(1, $_CFG['shop_country']));
	...
	$total = order_fee($order, $cart_goods, $consignee,$shopping,$d_discount);
...
elseif ($_REQUEST['step'] == 'change_surplus')
{
    ...
    else
    {
	    ...
	    $order['surplus_info'] = $surplus_info;
  	    ...
  	    $result['surplusnum'] = $order['surplus'];
            $result['content'] = $smarty->fetch('library/order_total.lbi');
        }
    }
...
elseif ($_REQUEST['step'] == 'done')
{
   ...
   /* 判断是不是实体商品 */
    ...
    $order['order_amount']  = ($total['amount'] > 0 ? number_format($total['amount'], 2, '.', '') : 0);
    ...
    $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
                "order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, cost_price,promote_price, ".
                "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, package_attr_id) ".
            " SELECT '$new_order_id', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, cost_price,promote_price,  ".
                "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, package_attr_id ".
            " FROM " .$ecs->table('cart') .
	...
	{
		send_sms($supplier_ids,'客户已付款，订单号为：ordersn请注意查看。【shopname】',2);				
	}
