 ...
 * @return  array
 */
function order_fee($order, $goods, $consignee, $shopping, $d_discount)
{
    /* 初始化订单的扩展code */
	...
	if($total['amount']>$shopping)
	{
		$total['amount'] -= $d_discount;
		$total['aa'] = 1;
		$total['dd'] = price_format($d_discount,false);
	}
    $total['amount_formated']  = price_format($total['amount'], false);
	$total['amount_formated_wap']  = $total['amount'];
